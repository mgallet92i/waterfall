...
Exactement **3 interactions HO** (INV-001) :
...
### Phase C — Checkpoint specs (interaction HO n°1)

Présenter le **contenu intégral de `specs.md`** via `AskUserQuestion`.
...
Charger le skill TL-light :

```
Agent(skill: "waterfall:wf-tl-light", brief: <brief passe 1>)
```

TL complétera les steps tl-owned (GENERATE_DESIGN, GENERATE_TASKS). PM supervise depuis
l'output de l'Agent call.

### Phase E — Checkpoint tasks (interaction HO n°2)

Présenter `tasks.md` complet via `AskUserQuestion`.
...
```
Agent(skill: "waterfall:wf-tl-light", brief: <brief passe 2>)
```
...
### Phase G — Validation finale (interaction HO n°3)
...
1. **Exactement 3 `AskUserQuestion` métier** (checkpoint-specs, checkpoint-tasks, validation-finale). (TF-006)
