...
- ✅ **RISQUE AUTH — levé (2026-06-19, Lot 2b point 1)** : sonde live `name=or` + `subagent_type=waterfall:wf-or` → le hook `wf-auth` a résolu le rôle `OR` et appliqué sa restriction (`wf-auth: OR write blocked on applicative path`). `_wf_normalize_agent_type` absorbe **les deux formes** (`waterfall:wf-or`→strip→`or` ; `or`→pass-through→`or`), donc l'auth résout tant que le `name` du spawn = rôle canonique (ce que les skills imposent). **Aucun changement d'auth requis pour la nouvelle API.** Confirmé aussi : livraison coéquipier→chef et `idle_notification` auto **observées en contexte** (messages `probe-tm` reçus), pas seulement déduites de la doc.
...
**Lot 2b — Thème A : coordination personas (2026-06-19, livré)** : retrait du **protocole ACK de livraison** (redondant avec la livraison native — sonde Phase 0). Constitution : section « Protocole ACK » → « Livraison des messages (native) ». Personas (`wf-or`, `wf-pm`, + 6 subordonnés `po/tl/rv/qa/dv/ds`) : suppression des blocs ACK (réception « ACK avant traitement », émission `--ack-register`/retry, `ACK-FIRST`, `INV-DISPATCH-ACK`, exemples), du `+ --ack-register` sur `brief_complete`, des lignes de table `ack:<msg_id>`/`Mandatory ACK`, reformulation des « Silence = accepted ». **Conservé** (→ Thème B) : `stuck_peer` (primitif d'escalade, partagé silent-subordinate/codewrite), INV-OR-POLL (découplé d'ACK), les commandes `--ack-*` du script + watchdog (retirées en B avec ack-registry). Découplage propre : aucune référence pendante au protocole supprimé. Tests : doc-drift 5/5.

**Lot 2b — Thème B : watchdog idle + plomberie ACK script (2026-06-19, livré)** : retrait conjoint du producteur (commandes `--ack-*`) et des consommateurs (détection idle), pour rester cohérent. Décision HO : **watchdog cron gardé pour crash/heartbeat**, **INV-OR-POLL gardé** (silent-subordinate, cas distinct du crash).
- `wf-watchdog.sh` : suppression `ACTOR_IDLE` + `idle_post_step_advanced` (+ lecture `ack-registry`, état `actor_idle_ticks`/`last_actor`) ; **gardé** `HEARTBEAT_MISSING/STALE` + `HISTORY_STAGNANT`. `scripts/test-watchdog-v3.sh` (12/13 cas = ACTOR_IDLE) **supprimé** avec la fonctionnalité (signalé HO : retire la seule couverture heartbeat de ce script).
