...
**Remède — 2 chantiers indépendants** :
  - **(1) Fix racine cwd [P0]** — `wf-orchestrate.sh` ne doit plus résoudre un projet fantôme ni renvoyer un défaut silencieux :
  ...
  - **(2) Chemin du script — migration B1 [P1, secondaire]** : enregistrer `config.orchestrate_path` (absolu, auto-déterminé par `SCRIPT_DIR` à `--init`) dans `.wf-state.json` et migrer les 158 `${CLAUDE_PLUGIN_ROOT}/scripts/wf-orchestrate.sh` pour le lire. Traite le risque `CLAUDE_PLUGIN_ROOT` UNSET (échec qui serait bruyant, pas la cause du silence). Décidé : **B1** (migration complète).
  - **Garde anti-silence (constitution)** : tout appel `wf-orchestrate.sh` d'un agent **vérifie l'exit code** et remonte une erreur explicite si ≠ 0. Convertit tout échec résiduel en panne visible.
