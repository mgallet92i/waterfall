...
```bash
pending=$(bash scripts/wf-orchestrate.sh <name> --ack-query --from rv)
now=$(date +%s)
```
...
```json
{"type":"stuck_peer","target":"<dest>","msg_id":"<id>","summary":"RV emitted <type> <topic>, 3 retries without ACK","attempts":3,"first_sent_at":"<iso>","last_retry_at":"<iso>"}
```
...
bash scripts/wf-orchestrate.sh <name> --ack-register --from rv --to or \
...
bash scripts/wf-orchestrate.sh <name> --complete REVIEW:RV_REVIEW --params "verdict=CONVERGE"
```
...
2. Read state: bash scripts/wf-orchestrate.sh <name> --query
