...
Puis : `bash scripts/wf-orchestrate.sh <name> --ack-escalate --msg-id <id>`
...
- **Bash write** — ne jamais utiliser `Bash` pour écrire des fichiers (`echo > file`, `cat > file`, `tee`, heredoc `<<EOF >`). Utiliser les outils natifs `Write` et `Edit`. Exception unique : `bash scripts/wf-orchestrate.sh <name> --log --msg "..."` pour or.log.
...
- **Exception unique** : `bash scripts/wf-orchestrate.sh <name> --log --msg "..."` pour appender à `or.log` (RC-01).
